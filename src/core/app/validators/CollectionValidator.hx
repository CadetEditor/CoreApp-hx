...
@:meta(Event(type="core.app.events.CollectionValidatorEvent",name="validItemsChanged"))
class CollectionValidator extends AbstractValidator
{
	...
	private var _validType : Class<Dynamic>;
	private var _min : Int;
	private var _max : Int;
	private var oldCollection : Array<Dynamic>;
	
	public function new(collection : core.data.ArrayCollection = null, validType : Class<Dynamic> = null, min : Int = 1, max : Int = Int.MAX_VALUE)
    {
        super();
		oldCollection = [];
	...
	private function set_Collection(value : ArrayCollection) : ArrayCollection
	{
	...
	private function get_Collection() : ArrayCollection
	{
		return _collection;
    }
	...
	private function set_ValidType(value : Class<Dynamic>) : Class<Dynamic>
	{
	...
	private function get_ValidType() : Class<Dynamic>
	{
		return _validType;
    }
	...
	public function getValidItems() : Array<Dynamic>
	{
		if (_collection == null) return [];
		var validItems : Array<Dynamic> = ArrayUtil.filterByType(_collection.source, _validType);
	...
	{
		var validItems : Array<Dynamic> = getValidItems();
		...
		if (ArrayUtil.compare(oldCollection, validItems) == false) {
			oldCollection = validItems;dispatchEvent(new CollectionValidatorEvent(CollectionValidatorEvent.VALID_ITEMS_CHANGED, validItems));
        }
    }
}