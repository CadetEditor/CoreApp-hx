...
class UndoableCompoundOperation extends EventDispatcher implements IAsynchronousOperation implements IUndoableOperation
{
	...
	private var currentOperation : IUndoableOperation;
	private var _description : String = "Compound Operation";
	...
	{
		if (Lambda.indexOf(operations, operation) != -1) return;
	...
	private function update() : Void
	{
		if (direction == 1) {
			...
			if (Std.is(currentOperation, IAsynchronousOperation)) {
				...
				cast((currentOperation), IAsynchronousOperation).addEventListener(ErrorEvent.ERROR, operationErrorHandler);
			...
			if (Std.is(currentOperation, IAsynchronousOperation)) {
				var asynchronousOperation : IAsynchronousOperation = cast((currentOperation), IAsynchronousOperation);
				...
				cast((asynchronousOperation), IUndoableOperation).undo();
            } else {  
