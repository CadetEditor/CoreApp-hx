...
class LoadManifestsOperation extends CompoundOperation
{
	private var manifestsXML : FastXMLList;
	private var manifest : Manifest;
	
	public function new(manifestsXML : FastXMLList)
    {
        super();
	...
	override public function execute() : Void
	{
		...
		for (i in 0...manifestsXML.length()) {
			var manifestNode : FastXML = manifestsXML.get(i);
			var url : String = Std.string(manifestNode.nodes.url.get(0).node.text.innerData());
