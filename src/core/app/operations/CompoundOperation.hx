...
public var operations : Array<Dynamic>;private var _label : String = "Compound Operation";private var _customLabelSet : Bool = false;private var currentOperation : IOperation;public function new()
    {
        super();operations = [];
    }private function get_CurrentIndex() : Int{return Lambda.indexOf(operations, currentOperation);
    }public function addOperation(operation : IOperation) : Void{operations.push(operation);
    }public function execute() : Void{if (operations.length == 0) {AsynchronousUtil.dispatchLater(this, new Event(Event.COMPLETE));return;
        }update();
    }private function update() : Void{if (currentIndex == (operations.length - 1) || operations.length == 0) {dispatchEvent(new Event(Event.COMPLETE));return;
        }currentOperation = operations[currentIndex + 1];if (Std.is(currentOperation, IAsynchronousOperation)) {cast((currentOperation), IAsynchronousOperation).addEventListener(Event.COMPLETE, operationCompleteHandler);cast((currentOperation), IAsynchronousOperation).addEventListener(OperationProgressEvent.PROGRESS, operationProgressHandler);cast((currentOperation), IAsynchronousOperation).addEventListener(ErrorEvent.ERROR, operationErrorHandler);updateLabel();  //trace("Compound Operation. Executing child operation : " + currentOperation.label);  currentOperation.execute();
        }
        else {  //trace("Compound Operation. Executing child operation : " + currentOperation.label);  currentOperation.execute();update();
        }
    }private function operationErrorHandler(event : ErrorEvent) : Void{dispatchEvent(event);
    }private function operationProgressHandler(event : OperationProgressEvent) : Void{updateLabel();var progressPerOperation : Float = 1 / operations.length;var index : Int = Lambda.indexOf(operations, event.target);var progress : Float = (index * progressPerOperation) + (event.progress * progressPerOperation);dispatchEvent(new OperationProgressEvent(OperationProgressEvent.PROGRESS, progress));
    ...
    }private function updateLabel() : Void{if (_customLabelSet)             return;var oldValue : String = _label;if (oldValue != currentOperation.label) {_label = currentOperation.label;dispatchEvent(new PropertyChangeEvent("propertyChange_label", null, _label));
        }
    }private function set_Label(value : String) : String{_label = value;_customLabelSet = true;dispatchEvent(new PropertyChangeEvent("propertyChange_label", null, _label));
        return value;
