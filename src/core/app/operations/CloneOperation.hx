...
class CloneOperation extends EventDispatcher implements IAsynchronousOperation
{
	...
	public function new(item : Dynamic, plugins : Array<ISerializationPlugin> = null)
    {
        super();
		this.item = item;
		serializer = new Serializer();
		serializer.addEventListener(Event.COMPLETE, serializeCompleteHandler); serializer.addEventListener(SerializeProgressEvent.PROGRESS, serializeProgressHandler);
