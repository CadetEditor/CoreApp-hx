...
class ReadFileAndDeserializeOperation extends EventDispatcher implements IAsynchronousOperation
{
	...
	public function new(uri : URI, fileSystemProvider : IFileSystemProvider, plugins : Array<ISerializationPlugin> = null)
    {
        super();
		this.uri = uri;
	...
	private function readFileCompleteHandler(event : Event) : Void
	{
		...
		var xml : FastXML = cast((readFileOperation.bytes.readUTFBytes(readFileOperation.bytes.length)), XML);
	...
	{
		return "Read file and deserialize : " + uri.getFilename();
    }
