...
class SerializationUtil
{
	...
	private static var _result : FastXML;
	
	public function new()
    {
    }
	
	public static function serialize(obj : Dynamic) : EventDispatcher
	{
		...
		var plugins : Array<ISerializationPlugin> = new Array<ISerializationPlugin>();
	...
	private static function serializeCompleteHandler(event : Event) : Void
	{
		var serializeOperation : SerializeOperation = cast((event.target), SerializeOperation);
		_result = serializeOperation.getResult();
		_eventDispatcher.dispatchEvent(new Event(Event.COMPLETE));
    }
	
	public static function getResult() : FastXML
	{
		return _result;
    }
}