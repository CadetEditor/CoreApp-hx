...
class ParameterizedQName extends AbcQName
{
	...
	public function new(name : AbcQName, parameters : Array<Dynamic>)
    {
	...
	{
		if (parameters.length == 0) return Std.string(super) + " []";
		var s : String = Std.string(super) + " [";
		for (name in parameters) s += Std.string(name) + ", ";
