...
class MultiFileSystemProvider extends EventDispatcher implements IMultiFileSystemProvider
{
    public var fileSystem(get, never) : FileSystemNode;
    public var id(get, never) : String;
    public var label(get, never) : String;
	private var providers : Dynamic;
	...
	public function registerFileSystemProvider(provider : IFileSystemProvider, addToFileSystem : Bool = true) : Void
	{
		providers[provider.id] = provider;
		Reflect.setField(idTable, Std.string(provider), provider.id);
	...
	private function get_FileSystem() : FileSystemNode {
		return _fileSystem;
    }
	...
	public function traverseToDirectory(uri : URI) : ITraverseToDirectoryOperation {
		var provider : IFileSystemProvider = getProviderForURI(uri);
		var operation : ITraverseToDirectoryOperation = provider.traverseToDirectory(uri); operation.addEventListener(ErrorEvent.ERROR, errorHandler, false, 0, true);
		return operation;
    }
	...
	public function writeFile(uri : URI, data : ByteArray) : IWriteFileOperation {
		var provider : IFileSystemProvider = getProviderForURI(uri);
		var operation : IWriteFileOperation = provider.writeFile(uri, data); operation.addEventListener(ErrorEvent.ERROR, errorHandler, false, 0, true);
		return operation;
    }
	
	private function getProviderForURI(uri : URI) : IFileSystemProvider {
		var split : Array<Dynamic> = uri.path.split("/");
		var providerID : String = split[0];
		var provider : IFileSystemProvider = Reflect.field(providers, providerID);
		if (provider == null) {
	...
	private function repopulateDirectory(uri : URI, contents : Array<URI>) : Void
	{
		var fileSystemNode : FileSystemNode = try cast(_fileSystem.getChildWithData(uri.path, true), FileSystemNode) catch (e:Dynamic) null;
		if (fileSystemNode == null) return;
		var childrenToRemove : Array<Dynamic> = [];
		var childrenToAdd : Array<Dynamic> = [];
		var childNode : FileSystemNode; 
		var i : Int;
		var childURI : URI;
		for (fileSystemNode.children.length) {
			childNode = fileSystemNode.children[i];
			var remove : Bool = true;
			for (childURI in contents) {
		...
		for (contents.length) {
			childURI = contents[i];
			var add : Bool = true;
			for (childNode/* AS3HX WARNING could not determine type for var: childNode exp: EField(EIdent(fileSystemNode),children) type: null */ in fileSystemNode.children) {
				if (childNode.path == childURI.path) {
					add = false;
                }
            }
	...
	{
		var node : FileSystemNode = try cast(_fileSystem.getChildWithData(uri.path, true), FileSystemNode) catch (e:Dynamic) null;
		if (node != null) return;
		var parentURI : URI = new URI();
		parentURI.copyURI(uri);
		(parentURI.isDirectory()) ? parentURI.chdir("../") : parentURI.chdir("./");
		var parentNode : FileSystemNode = try cast(_fileSystem.getChildWithData(parentURI.path, true), FileSystemNode) catch (e:Dynamic) null;
		if (parentNode == null) return;
		node = new FileSystemNode();
		node.path = uri.path;parentNode.children.addItem(node);
    }
	...
	{
		dispatchEvent(event);event.operation.addEventListener(Event.COMPLETE, operationCompleteHandler);
    }
	
	private function operationCompleteHandler(event : Event) : Void
	{
		var operation : IFileSystemProviderOperation = cast((event.target), IFileSystemProviderOperation);
		if (Std.is(event.target, ICreateDirectoryOperation)) {
			createDirectoryComplete(operation);
        }
        else if (Std.is(event.target, IDeleteFileOperation)) {
			deleteComplete(operation);
        }
        else if (Std.is(event.target, IDoesFileExistOperation)) {  
			//doesFileExistComplete( operation );  
        }
        else if (Std.is(event.target, IGetDirectoryContentsOperation)) {
			getDirectoryContentsComplete(operation);
        }
        else if (Std.is(event.target, ITraverseToDirectoryOperation)) {
			traverseToDirectoryComplete(operation);
        }
        else if (Std.is(event.target, ITraverseAllDirectoriesOperation)) {
			traverseAllDirectoriesComplete(operation);
        }
        else if (Std.is(event.target, IWriteFileOperation)) {
	...
	{
		var getDirectoryContentsOperation : IGetDirectoryContentsOperation = cast((operation), IGetDirectoryContentsOperation);
	...
	private function traverseToDirectoryComplete(operation : IFileSystemProviderOperation) : Void
	{
		...
		var allContents : Array<Dynamic> = traverseToDirectoryOperation.contents;
		for (i in 0...allContents.length) {
			var uri : URI = allContents[i].uri;
			var contents : Array<URI> = allContents[i].contents;repopulateDirectory(uri, contents);
        }
    }
	
	private function traverseAllDirectoriesComplete(operation : IFileSystemProviderOperation) : Void
	{
		var traverseOperation : ITraverseAllDirectoriesOperation = cast((operation), ITraverseAllDirectoriesOperation);
		var allContents : Array<Dynamic> = traverseOperation.contents;
		for (i in 0...allContents.length) {
			var uri : URI = allContents[i].uri;
			var contents : Array<URI> = allContents[i].contents;repopulateDirectory(uri, contents);
        }
