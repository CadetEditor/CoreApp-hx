...
class GetDirectoryContentsOperation extends EventDispatcher implements IGetDirectoryContentsOperation
{
	...
	private var _contents : Array<URI>;
	private var _defaultContentsXMLURL : String = "_contents.xml";
	
	@:allow(core.app.managers.filesystemproviders.url)
	...
	public function execute() : Void
	{
		...
		request.contentType = URLLoaderDataFormat.TEXT; loader.addEventListener(IOErrorEvent.IO_ERROR, errorHandler);
	...
	private function loadCompleteHandler(event : Event) : Void
	{
		...
		var contentsXML : FastXML;
		
		try {
			contentsXML = cast((loader.data), XML);
        }        
		catch (e : Error) {
			_contents = new Array<URI>();
			dispatchEvent(new Event(Event.COMPLETE));
		...
		_contents = new Array<URI>();
		for (i in 0...contentsXML.node.children.innerData().length()) {
			var child : FastXML = contentsXML.nodes.children()[i];
			if (child.node.name.innerData() == "file") {
				_contents.push(new URI(_uri.path + Std.string(child.node.text.innerData())));
            }
            else if (child.node.name.innerData() == "folder") {
				_contents.push(new URI(_uri.path + Std.string(child.node.text.innerData() + "/")));
            }
        }
	...
	private function get_Contents() : Array<URI>
	{
		return _contents;
    }
}