...
import nme.events.ErrorEvent; import nme.events.Event;
...
class DoesFileExistOperation extends EventDispatcher implements IDoesFileExistOperation
{
	...
	private var _fileExists : Bool = false;
	...
	{
		var getDirectoryContents : GetDirectoryContentsOperation = cast((event.target), GetDirectoryContentsOperation);  
		// Remove the FileSystemProvider's id from the start of the path;  
		var uriToFind : URI = _uri.subpath(1);
		_fileExists = false;
		for (uriInFolder/* AS3HX WARNING could not determine type for var: uriInFolder exp: EField(EIdent(getDirectoryContents),contents) type: null */ in getDirectoryContents.contents) {
	...
	private function get_FileExists() : Bool
	{
		return _fileExists;
    }
