...
import nme.events.Event;import core.app.core.managers.filesystemproviders.IFileSystemProvider;import core.app.core.managers.filesystemproviders.operations.ITraverseAllDirectoriesOperation;import core.app.entities.URI;import core.app.operations.CompoundOperation;class TraverseAllDirectoriesOperation extends CompoundOperation implements ITraverseAllDirectoriesOperation
{
...
private var _uri : URI;private var _fileSystemProvider : URLFileSystemProvider;private var _baseURL : String;private var _contents : Array<Dynamic>;public function new(uri : URI, fileSystemProvider : URLFileSystemProvider, baseURL : String)
    {
        super();_uri = uri;_fileSystemProvider = fileSystemProvider;_baseURL = baseURL;_contents = [];var operation : GetDirectoryContentsOperation = new GetDirectoryContentsOperation(uri, _fileSystemProvider, _baseURL);addOperation(operation);
                    ...
                    });for (i in 0...operation.contents.length){var uri : URI = operation.contents[i];if (uri.isDirectory()) {var newOperation : GetDirectoryContentsOperation = new GetDirectoryContentsOperation(uri, _fileSystemProvider, _baseURL);addOperation(newOperation);
                }
            }
