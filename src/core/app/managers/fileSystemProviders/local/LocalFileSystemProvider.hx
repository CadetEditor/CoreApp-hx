...
class LocalFileSystemProvider extends EventDispatcher implements ILocalFileSystemProvider
{
	...
	public function new(id : String, label : String, rootDirectory : File, defaultDirectory : File)
    {
        super();
		_id = id;
		_label = label;
	...
	private function get_RootDirectoryURI() : URI
	{		
		return new URI(_rootDirectory.url);
    }
	
	private function get_DefaultDirectoryURI() : URI
	{
		return new URI(_defaultDirectory.url);
    }
}