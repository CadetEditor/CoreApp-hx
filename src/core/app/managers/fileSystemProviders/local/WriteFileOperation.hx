...
import nme.events.OutputProgressEvent;
import nme.filesystem.File;
import nme.filesystem.FileMode;
import nme.filesystem.FileStream;
...
class WriteFileOperation extends LocalFileSystemProviderOperation implements IWriteFileOperation
{
	...
	{
		if (fileStream != null) {
			fileStream.close(); fileStream.removeEventListener(OutputProgressEvent.OUTPUT_PROGRESS, writeProgressHandler);
